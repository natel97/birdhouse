...
const generateEntities = (json) => {
  ...
  Object.keys(entities).forEach((key) => (map[key] = parseEntityData(entities[key], key)));
...
const generateServices = (json) => {
  ...
  const map = {};
  Object.keys(entities).forEach((e) => (map[e] = parseServiceData(e, entities[e])));
...
const generateControllers = (json) => {
  ...
  const hash = {};
  Object.keys(entities).forEach((key) => (hash[key] = parseControllerData(key, entities[key], routes[key])));
  return hash;
};

const createApplication = (entities, services, controllers, api) => {
  fs.mkdirSync(`${process.cwd()}/${api.name}`);
  fs.mkdirSync(`${process.cwd()}/${api.name}/src`);

  createSetup(api);

  Object.keys(entities).forEach((name) => {
    try {
      fs.mkdirSync(`${process.cwd()}/${api.name}/src/${name}`);
    } catch (e) {}
    fs.writeFileSync(`${process.cwd()}/${api.name}/src/${name}/${name}.entity.ts`, entities[name]);
    fs.writeFileSync(`${process.cwd()}/${api.name}/src/${name}/${name}.module.ts`, generateModule(name));
  });
  Object.keys(services).forEach((name) => {
    try {
      fs.mkdirSync(`${process.cwd()}/${api.name}/src/${name}`);
    } catch (e) {}
    fs.writeFileSync(`${process.cwd()}/${api.name}/src/${name}/${name}.service.ts`, services[name]);
  });
  Object.keys(controllers).forEach((name) => {
    try {
      fs.mkdirSync(`${process.cwd()}/${api.name}/src/${name}`);
    } catch (e) {}
    fs.writeFileSync(`${process.cwd()}/${api.name}/src/${name}/${name}.controller.ts`, controllers[name]);
  });
};
...
const construct = async () => {
  ...
  await createErFile(json);
  return nesting(json.api.name);
};
